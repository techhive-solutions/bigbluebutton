...
module BigBlueButton
  module Events
    ...
    def self.get_external_meeting_id(events_xml)
      ...
      external_meeting_id = {}
      external_meeting_id = metadata['meetingId'] unless metadata['meetingId'].nil?
      external_meeting_id
    end
    ...
    def self.find_video_event_matched(start_events, stop)
      ...
      start_events.each do |start|
        return start if start[:stream] == stop[:stream]
      end
      nil
    end
    ...
    def self.extract_filename_from_userId(userId, filenames_list)
      ...
      filenames_list.each do |filename|
        filename_return = filename unless filename.match(userId).nil?
      end
      filename_return
    end
    ...
    def self.process_webcamsOnlyForModerator(list_user_info, active_videos, inactive_videos, webcamsOnlyForModerator)
      if webcamsOnlyForModerator
        list_user_info.each do |user_id, _user_role|
          # If the user is a viewer:
          next if BigBlueButton::Events.is_user_moderator(user_id, list_user_info)
      ...
      if show_moderator_viewpoint
        ...
        events.xpath('/recording/event[@module="WEBCAM" or (@module="bbb-webrtc-sfu" and (@eventname="StartWebRTCShareEvent" or @eventname="StopWebRTCShareEvent")) or (@module="PARTICIPANT" and (@eventname="ParticipantStatusChangeEvent" or @eventname="ParticipantJoinEvent")) or @eventname="WebcamsOnlyForModeratorEvent" or @eventname="MeetingConfigurationEvent"]').each do |event|
          ...
          when 'StartWebcamShareEvent', 'StartWebRTCShareEvent'
            ...
            if !is_in_forbidden_period || (is_in_forbidden_period && BigBlueButton::Events.is_user_moderator(userId,
                                                                                                             list_user_info))
          ...
          when 'WebcamsOnlyForModeratorEvent'
            ...
            BigBlueButton::Events.process_webcamsOnlyForModerator(list_user_info, active_videos, inactive_videos,
                                                                  BigBlueButton::Events.to_boolean(event.at_xpath('webcamsOnlyForModerator').text))
    ...
    def self.get_matched_start_and_stop_deskshare_events(events)
      ...
      BigBlueButton::Events.match_start_and_stop_deskshare_events(
        ...
        when 'DeskshareStoppedEvent', 'StopWebRTCDesktopShareEvent'
          ...
          unless duration.nil?
            ...
            deskshare_edl.each do |entry|
              next if entry[:areas][:deskshare].nil?

              entry[:areas][:deskshare].each do |file|
                file[:original_duration] = duration * 1000 if file[:filename] == filename
              end
            end
          end
    ...
    def self.edl_entry_offset_audio
      proc do |edl_entry, offset|
        ...
        new_entry[:original_duration] = edl_entry[:original_duration] if edl_entry[:original_duration]
        new_entry
      end
    end
    ...
    def self.edl_match_recording_marks_audio(edl, events, start_time, end_time)
      ...
      BigBlueButton::Events.edl_match_recording_marks(edl, events,
    ...
    def self.edl_apply_start_stop_events(edl, edl_entry_offset, edl_empty_entry, start_stop_events)
      ...
      raise 'Recording stop event is after last EDL entry' if edl.last[:timestamp] < start_stop_events.last[:stop_timestamp]
    ...
    def self.edl_match_recording_marks(edl, events,
      ...
      BigBlueButton::Events.edl_apply_start_stop_events(edl, edl_entry_offset, edl_empty_entry, start_stop_events)
    end
    ...
    def self.trim_start_and_stop_rec_events(rec_events, start, stop)
      ...
      rec_events.each do |event|
        next if event[:start_timestamp] <= start and event[:stop_timestamp] <= start
        next if event[:start_timestamp] >= stop and event[:stop_timestamp] >= stop

        new_event = {
          start_timestamp: event[:start_timestamp],
          stop_timestamp: event[:stop_timestamp],
        }
        new_event[:start_timestamp] = start if new_event[:start_timestamp] < start
        new_event[:stop_timestamp] = stop if new_event[:stop_timestamp] > stop
    ...
    def self.get_segment_start_timestamp(events_xml, break_timestamp)
      ...
      if segment_i > 0
        ...
        event.at_xpath('breakTimestamp').text.to_i
      else
        # This is the first (or only) segment, so return the timestamp of
        # recording start (first event)
        BigBlueButton::Events.first_event_timestamp(events_xml)
      end
    end
    ...
    def self.bbb_version_compare(events, major, minor = nil, micro = nil)
      ...
      raise "bbb_version #{bbb_version} is not in the correct format" unless match

      # Check major version mismatch
      return true if match[1].to_i > major
      return false if match[1].to_i < major

      # Check minor version mismatch
      if minor.nil?
        return true
      else
        return true if match[2].to_i > minor
        return false if match[2].to_i < minor
      end
